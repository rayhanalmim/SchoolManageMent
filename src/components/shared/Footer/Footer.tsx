...
import { useGetFooterDataQuery } from "@/redux/api/baseApi";
...
const Footer = () => {
  ...
  } = useGetFooterDataQuery("footerData");
              ...
              <div className="text-center md:text-center lg:text-left">
                <Link href={"/"}>
                  <h2 className="font-bold text-2xl">
                    {footerData?.data.school_name}
                  </h2>
                </Link>
                <p className="mt-2 text-sm leading-6 text-gray-600">
                  {footerData?.data.short_description}
                </p>
              </div>
              <div className="flex items-center space-x-2">
                <MapPinIcon className="h-5 w56 text-gray-600" />
                <p className="text-sm leading-6 text-gray-600">
                  {footerData?.data.location}
                </p>
              </div>
              <div className="flex items-center space-x-2">
                <DevicePhoneMobileIcon className="h-5 w-5 text-gray-600" />
                <p className="text-sm leading-6 text-gray-600">
                  {footerData?.data.contact_number}
                </p>
              </div>
              <div className="flex items-center space-x-2">
                <EnvelopeIcon className="h-5 w-5 text-gray-600" />
                <p className="text-sm leading-6 text-gray-600">
                  {footerData?.data.email_address}
                </p>
              </div>
            </div>
          </div>
          <div className="mt-16 grid grid-cols-2 gap-8 xl:col-span-2 xl:mt-0">
            <div className="md:grid md:grid-cols-2 md:gap-8">
              <div>
                ...
                <ul
                  role="list"
                  className="lg:mt-6 lg:space-y-4 grid grid-cols-1 md:grid-cols-1 lg:grid-cols-1"
                >
        ...
        <div className="md:flex lg:flex justify-center content-center md:justify-between lg:justify-between border-t border-gray-900/10 pt-2">
          <div className="md:flex lg:flex md:gap-3 lg:gap-4 justify-center items-center content-center">
            <span className="mb-2 lg:mb-0">
              &copy; {new Date().getFullYear()} SoftEdu. All rights reserved.
            </span>
